...
namespace kinematics_library
{
    ...
    int Ik7DoFSolver::fkArm()
    {
        ...
        // for(i = 0; i < 7; i++)
        {
            ...
            T_Fr_2_Fr[4] = -st * ca;
            T_Fr_2_Fr[8] = st * sa;
            T_Fr_2_Fr[12] = ct * arm_->dh_a[i];
            T_Fr_2_Fr[1] = st;
            T_Fr_2_Fr[5] = ct * ca;
            T_Fr_2_Fr[9] = -ct * sa;
            T_Fr_2_Fr[13] = st * arm_->dh_a[i];
            T_Fr_2_Fr[2] = 0.0;
            T_Fr_2_Fr[6] = sa;
            T_Fr_2_Fr[10] = ca;
            T_Fr_2_Fr[14] = arm_->dh_d[i];
            T_Fr_2_Fr[3] = 0.0;
            T_Fr_2_Fr[7] = 0.0;
            T_Fr_2_Fr[11] = 0.0;
            T_Fr_2_Fr[15] = 1.0;
    ...
    int Ik7DoFSolver::ikArm()
    {
        ...
        else
        {
            ...
            double sqrt_term = sqrt((w_t[0] * w_t[0] + w_t[1] * w_t[1]) * (lf * lf * (-(lf * lf) + 2.0 * lu * lu + 2.0 * d * d) + lu * lu * (-lu * lu + 2.0 * d * d) + w_t[0] * w_t[0] * (-w_t[0] * w_t[0] - 2.0 * w_t[1] * w_t[1] - 2.0 * w_t[2] * w_t[2]) + w_t[1] * w_t[1] * (-w_t[1] * w_t[1] - 2 * w_t[2] * w_t[2]) - w_t[2] * w_t[2] * w_t[2] * w_t[2]));
            ...
            for (j = 0; j < 2; j++)
            {
                ...
                if (elbowIsStraight)
                {
                    ...
                    for (q = 0; q < 4; q++)
                    {
                        ...
                        T_Fr_2_Fr[4] = -st * ca;
                        T_Fr_2_Fr[8] = st * sa;
                        T_Fr_2_Fr[12] = ct * arm_->dh_a[q];
                        T_Fr_2_Fr[1] = st;
                        T_Fr_2_Fr[5] = ct * ca;
                        T_Fr_2_Fr[9] = -ct * sa;
                        T_Fr_2_Fr[13] = st * arm_->dh_a[q];
                        T_Fr_2_Fr[2] = 0.0;
                        T_Fr_2_Fr[6] = sa;
                        T_Fr_2_Fr[10] = ca;
                        T_Fr_2_Fr[14] = arm_->dh_d[q];
                        T_Fr_2_Fr[3] = 0.0;
                        T_Fr_2_Fr[7] = 0.0;
                        T_Fr_2_Fr[11] = 0.0;
                        T_Fr_2_Fr[15] = 1.0;
                ...
                // angles
                double ct8 = cos(arm_->dh_do[7]);
                double ca8 = arm_->dh_ca[7];
                double st8 = sin(arm_->dh_do[7]);
                double sa8 = arm_->dh_sa[7];
                double T_78[16];
                double T_78_inv[16];
                double T_08[16];
                double T_07[16];
    ...
    base::samples::RigidBodyState Ik7DoFSolver::getRBSPose(double *homogeneous_mat)
    {
        ...
        rot_mat(0, 0) = homogeneous_mat[0];
        rot_mat(0, 1) = homogeneous_mat[4];
        rot_mat(0, 2) = homogeneous_mat[8];
        rot_mat(1, 0) = homogeneous_mat[1];
        rot_mat(1, 1) = homogeneous_mat[5];
        rot_mat(1, 2) = homogeneous_mat[9];
        rot_mat(2, 0) = homogeneous_mat[2];
        rot_mat(2, 1) = homogeneous_mat[6];
        rot_mat(2, 2) = homogeneous_mat[10];
