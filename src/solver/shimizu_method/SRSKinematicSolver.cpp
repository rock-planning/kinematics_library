...
namespace kinematics_library
{
    ...
    base::samples::RigidBodyState SRSKinematicSolver::direct(const std::vector<double> &joint_angles)
    {
        ...
        fk[0][6] = sa[0];
        fk[0][10] = ca[0];
        fk[0][14] = srs_config_.dh_param.link_offsets[0];
        fk[0][3] = 0;
        fk[0][7] = 0;
        fk[0][11] = 0;
        fk[0][15] = 1;
        ...
        for (int i = 1; i < 7; i++)
        {
            ...
            tmpMat[6] = sa[i];
            tmpMat[10] = ca[i];
            tmpMat[14] = srs_config_.dh_param.link_offsets[i];
        ...
        fk_pose.position(0) = fk[6][12];
        fk_pose.position(1) = fk[6][13];
        fk_pose.position(2) = fk[6][14];
    ...
    int SRSKinematicSolver::invkin(const base::Position &pos, const base::Quaterniond &rot, base::commands::Joints &jointangles)
    {
        ...
        else
        {
            ...
            thet4 = ((t_thet4 - (srs_config_.dh_param.link_offsets[2] * srs_config_.dh_param.link_offsets[2]) - (srs_config_.dh_param.link_offsets[4] * srs_config_.dh_param.link_offsets[4])) /
            ...
            usw_sk.at(0) = 0.0;
            usw_sk.at(3) = -usw.at(2);
            usw_sk.at(6) = usw.at(1);
            usw_sk.at(1) = usw.at(2);
            usw_sk.at(4) = 0.0;
            usw_sk.at(7) = -usw.at(0);
            usw_sk.at(2) = -usw.at(1);
            usw_sk.at(5) = usw.at(0);
            usw_sk.at(8) = 0.0;
    ...
    std::string SRSKinematicSolver::printError(int err)
    {
        ...
        switch (err)
        {
        case SRSKinematic::SUCCESS:
            error_message = " Kinematic success ";
            break;
        case SRSKinematic::ERR_BOUND_J1:
            error_message = " Boundary condition failed in Joint 1 ";
            break;
        case SRSKinematic::ERR_BOUND_J2:
            error_message = " Boundary condition failed in Joint 2 ";
            break;
        case SRSKinematic::ERR_BOUND_J3:
            error_message = " Boundary condition failed in Joint 3 ";
            break;
        case SRSKinematic::ERR_BOUND_J5:
            error_message = " Boundary condition failed in Joint 5 ";
            break;
        case SRSKinematic::ERR_BOUND_J6:
            error_message = " Boundary condition failed in Joint 6 ";
            break;
        case SRSKinematic::ERR_BOUND_J7:
            error_message = " Boundary condition failed in Joint 7 ";
            break;
        case SRSKinematic::ERR_REACH:
            error_message = " Pose not reachable ";
            break;
        case SRSKinematic::ERR_TRANS_EQU:
            error_message = " Error in solving the transcendental equation - the sqaure root is complex ";
            break;
        case SRSKinematic::ERR_TRANS_EQU_COND:
            error_message = " A condition arised while solving the transcendental equation - It need to be studied ";
            break;
        case SRSKinematic::ERR_TRANS_EQU_PICK_1:
            error_message = " A condition (cond 1) arised while picking a solution from the result obatined through transcendental equation - It need to be studied ";
            break;
        case SRSKinematic::ERR_TRANS_EQU_PICK_2:
            error_message = " A condition (cond 2) arised while picking a solution from the result obatined through transcendental equation - It need to be studied ";
            break;
        case SRSKinematic::ERR_TRANS_EQU_PICK_3:
            error_message = " A condition (cond 3) arised while picking a solution from the result obatined through transcendental equation - It need to be studied ";
            break;
        case SRSKinematic::ERR_TRANS_EQU_PICK_4:
            error_message = " A condition (cond 4) arised while picking a solution from the result obatined through transcendental equation - It need to be studied ";
            break;
        case SRSKinematic::ERR_TRANS_EQU_PICK_5:
            error_message = " A condition (cond 5) arised while picking a solution from the result obatined through transcendental equation - It need to be studied ";
            break;
        case SRSKinematic::ERR_PSI_REARRANGE_RANGE:
            error_message = " Error while rearranging the psi range - It need to be studied ";
            break;
        case SRSKinematic::ERR_UNION_SINGLE:
            error_message = " Error in calculating the union of joints with only one feasible armangle ";
            break;
        case SRSKinematic::ERR_COMPLIMENT:
            error_message = " Error in calculating the complement of infeasible arm angle ";
            break;
        case SRSKinematic::ERR_UNION_ALL:
            error_message = " There is no intersection in the feasible armangles ";
            break;
        default:
            error_message = " This error message is not defined";
        }
