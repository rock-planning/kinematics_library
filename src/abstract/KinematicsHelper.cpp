...
namespace kinematics_library
{
    ...
    void zyxEuler2rotMat(const std::vector<double> &eul_zyx, Eigen::Matrix3d &rot_mat)
    {
        ...
        rot_mat(0, 0) = ca * cb;
        rot_mat(0, 1) = (ca * sb * sg) - (sa * cg);
        rot_mat(0, 2) = (ca * sb * cg) + (sa * sg);
        rot_mat(1, 0) = sa * cb;
        rot_mat(1, 1) = (sa * sb * sg) + (ca * cg);
        rot_mat(1, 2) = (sa * sb * cg) - (ca * sg);
        rot_mat(2, 0) = -sb;
        rot_mat(2, 1) = cb * sg;
        rot_mat(2, 2) = cb * cg;
    }
    ...
    void rotationMatrix2zyxEuler(const double *&rotMat, Eigen::Vector3d &zyxEuler)
    {
        ...
        if (zyxEuler(1) < -M_PI / 2.0 + 0.001)
        { // ca. -PI/2
            zyxEuler(0) = 0.0;
            zyxEuler(1) = -M_PI / 2.0;
            zyxEuler(2) = -atan2(rotMat[1], rotMat[4]);
        }
        else if (zyxEuler(1) > M_PI / 2.0 - 0.001)
        { // ca. PI/2
            zyxEuler(0) = 0.0;
            zyxEuler(1) = M_PI / 2.0;
            zyxEuler(2) = atan2(rotMat[1], rotMat[4]);
    ...
    {
        rot_mat(0, 0) = 1 - 2 * ((quat.y() * quat.y()) + (quat.z() * quat.z()));
        rot_mat(0, 1) = 2 * ((quat.x() * quat.y()) - (quat.z() * quat.w()));
        rot_mat(0, 2) = 2 * ((quat.x() * quat.z()) + (quat.y() * quat.w()));
        rot_mat(1, 0) = 2 * ((quat.x() * quat.y()) + (quat.z() * quat.w()));
        rot_mat(1, 1) = 1 - 2 * ((quat.x() * quat.x()) + (quat.z() * quat.z()));
        rot_mat(1, 2) = 2 * ((quat.y() * quat.z()) - (quat.x() * quat.w()));
        rot_mat(2, 0) = 2 * ((quat.x() * quat.z()) - (quat.y() * quat.w()));
        rot_mat(2, 1) = 2 * ((quat.y() * quat.z()) + (quat.x() * quat.w()));
        rot_mat(2, 2) = 1 - 2 * ((quat.x() * quat.x()) + (quat.y() * quat.y()));
    }
    ...
    void quaternionToRotationMatrixArray(const base::Quaterniond &quat, double *rot_mat)
    {
        ...
        rot_mat[0] = rot_matrix(0, 0);
        rot_mat[1] = rot_matrix(0, 1);
        rot_mat[2] = rot_matrix(0, 2);
        rot_mat[3] = rot_matrix(1, 0);
        rot_mat[4] = rot_matrix(1, 1);
        rot_mat[5] = rot_matrix(1, 2);
        rot_mat[6] = rot_matrix(2, 0);
        rot_mat[7] = rot_matrix(2, 1);
        rot_mat[8] = rot_matrix(2, 2);
    }
                                           ...
                                           const base::samples::RigidBodyState &source_pose, base::samples::RigidBodyState &target_pose)
    {
        ...
        if ((source_pose.targetFrame.compare(target_pose.targetFrame) != 0) && (!target_pose.targetFrame.empty()))
        {
            ...
            kinematics_library::rbsToKdl(target_pose, calculated_frame);
            new_frame = calculated_frame * fk_pose;
            kinematics_library::kdlToRbs(new_frame, target_pose);
        }
    ...
    void eul2RotMat(const double eul_zyx[3], std::vector<double> &rot_mat)
    {
        ...
        rot_mat.at(0) = ca * cb;
        rot_mat.at(3) = (ca * sb * sg) - (sa * cg);
        rot_mat.at(6) = (ca * sb * cg) + (sa * sg);
        rot_mat.at(1) = sa * cb;
        rot_mat.at(4) = (sa * sb * sg) + (ca * cg);
        rot_mat.at(7) = (sa * sb * cg) - (ca * sg);
        rot_mat.at(2) = -sb;
        rot_mat.at(5) = cb * sg;
        rot_mat.at(8) = cb * cg;
    }
    ...
    void quaternionToRotMat(const Eigen::Quaternion<double> &quat, std::vector<double> &rot_mat)
    {
        ...
        rot_mat.at(0) = mat(0, 0);
        rot_mat.at(3) = mat(0, 1);
        rot_mat.at(6) = mat(0, 2);
        rot_mat.at(1) = mat(1, 0);
        rot_mat.at(4) = mat(1, 1);
        rot_mat.at(7) = mat(1, 2);
        rot_mat.at(2) = mat(2, 0);
        rot_mat.at(5) = mat(2, 1);
        rot_mat.at(8) = mat(2, 2);
    }
    ...
    {
        res.at(0) = (mat1.at(0) * mat2.at(0)) + (mat1.at(3) * mat2.at(1)) + (mat1.at(6) * mat2.at(2));
        res.at(3) = (mat1.at(0) * mat2.at(3)) + (mat1.at(3) * mat2.at(4)) + (mat1.at(6) * mat2.at(5));
        res.at(6) = (mat1.at(0) * mat2.at(6)) + (mat1.at(3) * mat2.at(7)) + (mat1.at(6) * mat2.at(8));
        res.at(1) = (mat1.at(1) * mat2.at(0)) + (mat1.at(4) * mat2.at(1)) + (mat1.at(7) * mat2.at(2));
        res.at(4) = (mat1.at(1) * mat2.at(3)) + (mat1.at(4) * mat2.at(4)) + (mat1.at(7) * mat2.at(5));
        res.at(7) = (mat1.at(1) * mat2.at(6)) + (mat1.at(4) * mat2.at(7)) + (mat1.at(7) * mat2.at(8));
        res.at(2) = (mat1.at(2) * mat2.at(0)) + (mat1.at(5) * mat2.at(1)) + (mat1.at(8) * mat2.at(2));
        res.at(5) = (mat1.at(2) * mat2.at(3)) + (mat1.at(5) * mat2.at(4)) + (mat1.at(8) * mat2.at(5));
        res.at(8) = (mat1.at(2) * mat2.at(6)) + (mat1.at(5) * mat2.at(7)) + (mat1.at(8) * mat2.at(8));
    }
    ...
    {
        res.at(0) = vec.at(0) * vec.at(0);
        res.at(3) = vec.at(0) * vec.at(1);
        res.at(6) = vec.at(0) * vec.at(2);
        res.at(1) = vec.at(1) * vec.at(0);
        res.at(4) = vec.at(1) * vec.at(1);
        res.at(7) = vec.at(1) * vec.at(2);
        res.at(2) = vec.at(2) * vec.at(0);
        res.at(5) = vec.at(2) * vec.at(1);
        res.at(8) = vec.at(2) * vec.at(2);
    }
    ...
    void invertMat(double mat[16], double *res)
    {
        ...
        res[4] = t_mat[3];
        res[8] = t_mat[6];
        res[12] = new_pos_neg[0];
        res[1] = t_mat[1];
        res[5] = t_mat[4];
        res[9] = t_mat[7];
        res[13] = new_pos_neg[1];
        res[2] = t_mat[2];
        res[6] = t_mat[5];
        res[10] = t_mat[8];
        res[14] = new_pos_neg[2];
        res[3] = 0.0;
        res[7] = 0.0;
        res[11] = 0.0;
        res[15] = 1.0;
